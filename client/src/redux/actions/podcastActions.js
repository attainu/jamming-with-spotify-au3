...
export const createPodcast = podcastData => {
  return dispatch => {
    const request = new Request(`http://localhost:8888/podcasts`, {
      headers: new Headers({
...
export const fetchPodcastMenu = userName => {
  return dispatch => {
    const request = new Request(`http://localhost:8888/podcasts/${userName}`);
...
export const deletePodcast = podcastId => {
  return dispatch => {
    const request = new Request(`http://localhost:8888/podcasts/${podcastId}`, {
      method: "DELETE"
    });
